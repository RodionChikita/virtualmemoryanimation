...
class VirtualMemoryDemo(Scene):
    def construct(self):
        ...
        # 2) оранжевая рамка вокруг содержимого кучи
        heap_rect = RoundedRectangle(
            corner_radius=0.12,
            color=ORANGE,
            stroke_width=2
        )
        # ВАЖНО: в этой версии manim здесь параметр buff
        heap_rect.surround(heap_content, buff=0.25)

        # 3) заголовок сверху от рамки
        ...
        memory_group.move_to(RIGHT * 5.2 + DOWN * 0.3)

        # распакуем после масштабирования
        ...
        page_label.next_to(heap_segments[1], RIGHT, buff=0.12)
